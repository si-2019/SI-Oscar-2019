...
public class TestoviPrivilegija {
    ...
    @Test
    public void testUlogaImaPrivilegiju() throws IOException{

            Uloga prof=ulogaRepozitorij.findBynazivUloge(ImenaUloga.PROFESOR);
            Long idProf=prof.getId();
            if(privilegijaRepozitorij.findBynazivPrivilegije("registrovanje-casa")!=null){
                URL url = new URL("http://localhost:31915/uloga/"+idProf.toString()+"/registrovanje-casa");
                HttpURLConnection con = (HttpURLConnection) url.openConnection();
                con.setDoOutput(false);
                con.setRequestMethod("GET");
                InputStream in = con.getInputStream();
